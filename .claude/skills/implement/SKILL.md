...
- Respect the `commands → services → db` boundary — commands stay
  thin, services hold orchestration, db layer is the only SQL site
...
This runs: typecheck → lint → test:run → build. **All must pass.**
...
Never commit `.env` files, credentials, secrets, or session cookie
fixtures captured from a live Alko session.
...
- If `npm run test:all` does not pass, do NOT push or create PR
