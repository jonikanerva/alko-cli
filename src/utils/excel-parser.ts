...
export function parseAlkoExcel(buffer: ArrayBuffer): Product[] {
  ...
  logger.debug(`Found header row at index ${headerRowIndex}`);
...
export function validateProducts(products: Product[]): {
  ...
  for (const product of products) {
    ...
    if (product.price < 0) errors.push(`Invalid price: ${product.price}`);
    ...
    if (errors.length === 0) valid.push(product);
    else invalid.push({ product, errors });
  }
